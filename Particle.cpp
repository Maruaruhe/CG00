...
void Particle::Initialize(DirectX12* directX12) {
...
void Particle::Update(Vector4& color, Transform& transform_, DirectionalLight& direcionalLight) {
	...
	for (uint32_t index = 0; index < kNum; ++index) {
...
void Particle::Draw() {
	...
	directX12_->GetCommandList()->DrawInstanced(UINT(modelData.vertices.size()), kNum, 0, 0);
}
...
void Particle::CreateInstance() {
	instancingResource = directX12_->CreateBufferResource(directX12_->GetDevice(), sizeof(TransformationMatrix) * kNum);
...
void Particle::CreateSRV() {
	...
	instancingSrvDesc.Buffer.NumElements = kNum;
